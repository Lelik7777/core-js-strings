...
function removeLastOccurrences(str, value) {
  if (str.lastIndexOf(value) !== -1) {
    const copy = [...str];
    copy.splice(str.lastIndexOf(value), value.length);
    return copy.join('');
  }
...
function formatTime(minutes, seconds) {
  return `${String(minutes).length < 2 ? '0'.concat(minutes) : minutes}:${
    String(seconds).length < 2 ? '0'.concat(seconds) : seconds
  }`;
}
...
function isPalindrome(str) {
  return [...str].length <= 1
    ...
    : str.replace(/[\W_]/g, '').toLocaleLowerCase() ===
        str
          .replace(/[\W_]/g, '')
          .split('')
          .reverse()
...
function invertCase(str) {
  return str
    ...
    .map((word) =>
      [...word]
        .map((char, i, array) =>
          char === array[i].toLocaleUpperCase()
            ? char.toLowerCase()
            : char.toUpperCase()
        )
        .join('')
...
return value.split(' ').map((word,i)=>i>0?word:'')
} => 'John Doe'
...
function encodeToRot13(str) {
  ...
  // ?str.split(' ')
  // .map(word=>[...word].map(char=>encodeLetters.charAt(letters.indexOf(char)))
  // : [...str].map((char) => encodeLetters[letters.indexOf(char)]).join('');
  return str.split(' ').length > 1
    ? str
        .split(' ')
        .map((word) =>
          [...word]
            .map((char) =>
              char === '?' || char === '!'
                ? char
                : encodeLetters[letters.indexOf(char)]
            )
            .join('')
        )
        .join(' ')
